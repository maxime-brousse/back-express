...
module.exports = function(app, pool, authenticateToken) {
    app.post('/tournament/create', authenticateToken, (req, res) => {
        ...
        INSERT INTO tournoi (jeux, dateTournoi, descriptionTournoi, récompensePoint, isSolo, titreTournoi, nbJoueursMax)
        VALUES (?, ?, ?, ?, ?, ?, ?)
            `;

        const query = pool.query(insertTournoiSql, [jeux, formatedDateTournoi, descriptionTournoi, récompensePoint, isSolo, titreTournoi, nbJoueursMax], (err, result) => {
            if (err) {
